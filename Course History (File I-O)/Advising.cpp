...
int Advising::creditsRemaining() {
    ...
    for (int i = 0; i < curriculum.gisCourses.size(); i++) {
        ...
        if (courseSemester > currentSemester) {
            cout << curriculum.gisCourses[i].courseCode << " " << curriculum.gisCourses[i].courseName << " | Credits: "
                << curriculum.gisCourses[i].credits << endl;

            totalCreditsRemaining += curriculum.gisCourses[i].credits;
        }
    }

    cout << "Credits Remaining: " << totalCreditsRemaining << endl;
    return 0;
}
